...
export default function GraphView({ nodes, activeNodeId, onNodeClick, branchAnimation }: GraphViewProps) {
  ...
  // Initial center on first render with nodes - use useLayoutEffect to prevent flicker
  useLayoutEffect(() => {
    if (nodes.size > 0 && containerRef.current && !isInitialized) {
      const container = containerRef.current;
