...
export default function ChatNode({ node, isActive, onClick, scale = 1 }: ChatNodeProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`w-full h-full rounded-lg px-4 py-3 border-2 transition-all duration-300 shadow-md ${isActive
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 rounded-full ${isActive ? 'bg-blue-500' : node.isFrozen ? 'bg-gray-600' : 'bg-green-500'
                }`}
            />
