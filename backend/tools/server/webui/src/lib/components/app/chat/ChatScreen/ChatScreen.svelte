<script lang="ts">
	...
	// Branch mode state
	let isBranchMode = $state(false);
	let branchAnimationState = $state<'idle' | 'minimizing' | 'transitioning' | 'zooming'>('idle');
	let selectedTextForBranch = $state<{ messageId: string; text: string } | null>(null);
	let chatContainerScale = $state(1);
	let chatContainerOpacity = $state(1);
	...
	async function handleSendMessage(message: string, files?: ChatUploadedFile[]): Promise<boolean> {
		...
		// If in branch mode, handle branch creation
		if (isBranchMode && selectedTextForBranch) {
			await handleBranchCreation(message, extras);
			return true;
		}
	...
	async function handleBranchCreation(message: string, extras?: DatabaseMessageExtra[]) {
		if (!selectedTextForBranch) return;

		try {
			// Start animation sequence
			branchAnimationState = 'minimizing';

			// Animate chat minimize
			chatContainerScale = 0.8;
			chatContainerOpacity = 0.3;

			await new Promise((resolve) => setTimeout(resolve, 400));

			// Create branch with the user's message
			branchAnimationState = 'transitioning';
			const branchResult = await chatStore.createBranchFromMessage(
				selectedTextForBranch.messageId,
				selectedTextForBranch.text,
				message
			);
			...
			// Navigate to new branch conversation
			await goto(`#/chat/${branchResult.conversationId}`);

			// Zoom animation
			branchAnimationState = 'zooming';
			chatContainerScale = 0.8;
			chatContainerOpacity = 0;

			await new Promise((resolve) => setTimeout(resolve, 50));

			// Zoom in
			chatContainerScale = 1;
			chatContainerOpacity = 1;

			await new Promise((resolve) => setTimeout(resolve, 400));

			// Branch creation already sent the message and started generation
	...
	function handleBranch(message: DatabaseMessage, selectedText: string) {
		isBranchMode = true;
		selectedTextForBranch = {
			messageId: message.id,
			text: selectedText
		};

		// Auto-focus on text entry
		setTimeout(() => {
			// The form will auto-focus when entering branch mode
		}, 100);
	}

	function resetBranchMode() {
		isBranchMode = false;
		branchAnimationState = 'idle';
		selectedTextForBranch = null;
		chatContainerScale = 1;
		chatContainerOpacity = 1;
	}
...
{#if !isEmpty}
	<div
		...
		class="chat-container flex h-full flex-col overflow-y-auto px-4 md:px-6"
		style="transform: scale({chatContainerScale}); opacity: {chatContainerOpacity}; transition: transform 400ms ease-out, opacity 400ms ease-out;"
