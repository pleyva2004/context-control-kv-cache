...
struct server_task_result_cmpl_final : server_task_result {
    ...
    json to_json_oaicompat_chat_stream() {
        ...
        // Add slot_id to the final streaming chunk
        deltas.back()["slot_id"] = id_slot;
    ...
    */
    ...
    const auto handle_chat_branch = [&ctx_server, &res_error, &res_ok](const httplib::Request & req, httplib::Response & res) {
        ...
        if (data.contains("prompt") && data["prompt"].is_string()) {
            user_prompt = data["prompt"].get<std::string>();
        } else {
        ...
        json formatted_data;
        try {
            formatted_data = oaicompat_chat_params_parse(chat_body, ctx_server.oai_parser_opt, files);
        } catch (const std::exception & e) {
        ...
        task.id = ctx_server.queue_tasks.get_new_id();
        task.index = 0;  // Single task, use index 0
        task.params = params;
